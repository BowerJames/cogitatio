#!/usr/bin/env python3
"""
Train Baseline model (encoder-decoder only, no residual layers).

This is the simplest model - just embed tokens and decode directly.
Used as a lower bound for comparison with other architectures.

Usage:
    uv run python -m cogitatio.scripts.train_baseline --epochs 20
"""
...
from cogitatio.model import BaselineModel


def parse_args():
    parser = argparse.ArgumentParser(
        description="Train Baseline (encoder-decoder only) model",
...
def main():
    ...
    print("=" * 60)
    print("Baseline Model Training (Encoder-Decoder Only)")
    print("=" * 60)
    print(f"\nDevice: {device}")
