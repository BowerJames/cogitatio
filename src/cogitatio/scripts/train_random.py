#!/usr/bin/env python3
"""
Train Random Order model (all blocks applied in random order).

This is an ablation model that uses the same block pool as MuThought,
but applies them in a random order each forward pass rather than
learning which order is best.

Usage:
    uv run python -m cogitatio.scripts.train_random --epochs 20 --n_blocks 4
"""
...
from cogitatio.model import RandomOrderModel


def parse_args():
    parser = argparse.ArgumentParser(
        description="Train Random Order model",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    
    # Model architecture
    parser.add_argument("--d_model", type=int, default=64, help="Hidden dimension")
    parser.add_argument("--n_blocks", type=int, default=4, help="Number of thought blocks")
...
def main():
    ...
    print("=" * 60)
    print("Random Order Model Training")
    print("=" * 60)
    print(f"\nDevice: {device}")
